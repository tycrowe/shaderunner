...
export const eval_prompt = ({ url, title, query }) => {
    const SYSTEM = `Classify sentences from a webpage as either 'interesting' or 'neutral'.
For this, use two categories: 'interesting' for sentences closely related to specific topics relevant to the user query, and 'neutral' for sentences that are only mildly related or unrelated.
Sentences are deemed 'interesting' if they are nearer to a topic in the 'interesting' category than to those in the 'neutral' category.
Conversely, classify a sentence as 'neutral' if it is broadly related to general topics but not closely aligned with the 'interesting' topics.
...
Neutral Class Topics: General Health Tips|Meat Nutrition|Fish Nutrition|Advertisements|Testimonials|External Links
...
Neutral Class Topics: Hashing|String Matching|Algorithm History|Algorithm Theory|String Matching Algorithms|Usage|Related Algorithms|Code Expression|Programming in C++|Site Navigation|External Links
...
Neutral Class Topics: General Moon Landing History|Other Space Missions|Astronaut Biographies|Space Exploration Technology|Future Moon Missions|Advertisements|External Links|Site Navigation
...
export function parseInput(input: string) {
    ...
    const output = lines.map((line: string) => {
      ...
      } else if (line.includes('Neutral Class Topics:')) {
        line = line.replace('Neutral Class Topics:', '').trim();
        return line ? line.split("|") : [];
      }
      return line.trim()
